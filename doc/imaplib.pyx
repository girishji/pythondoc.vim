*imaplib.pyx*                                 Last change: 2023 Sep 15
...
exception IMAP4.error                       *IMAP4.error..imaplib.pyx*

   Exception raised on any errors.  The reason for the exception is
   passed to the constructor as a string.

exception IMAP4.abort                       *IMAP4.abort..imaplib.pyx*
...
exception IMAP4.readonly                 *IMAP4.readonly..imaplib.pyx*
...
class imaplib.IMAP4_stream(command)        *IMAP4_stream..imaplib.pyx*
...
The following utility functions are defined:
                                   ...
                                   *Internaldate2tuple()..imaplib.pyx*
...
imaplib.Int2AP(num)                            *Int2AP()..imaplib.pyx*

   Converts an integer into a bytes representation using characters
   from the set ["A" .. "P"].

imaplib.ParseFlags(flagstr)                *ParseFlags()..imaplib.pyx*

   Converts an IMAP4 "FLAGS" response to a tuple of individual flags.

                                    *Time2Internaldate()..imaplib.pyx*
...
An "IMAP4" instance has the following methods:
                                         ...
                                         *IMAP4.append()..imaplib.pyx*
IMAP4.append(mailbox, flags, date_time, message)

   Append _message_ to named mailbox.

                                   *IMAP4.authenticate()..imaplib.pyx*
...
IMAP4.check()                             *IMAP4.check()..imaplib.pyx*

   Checkpoint mailbox on server.

IMAP4.close()                             *IMAP4.close()..imaplib.pyx*

   Close currently selected mailbox. Deleted messages are removed from
   writable mailbox. This is the recommended command before "LOGOUT".

IMAP4.copy(message_set, new_mailbox)       *IMAP4.copy()..imaplib.pyx*

   Copy _message_set_ messages onto end of _new_mailbox_.

IMAP4.create(mailbox)                    *IMAP4.create()..imaplib.pyx*

   Create new mailbox named _mailbox_.

IMAP4.delete(mailbox)                    *IMAP4.delete()..imaplib.pyx*

   Delete old mailbox named _mailbox_.

IMAP4.deleteacl(mailbox, who)         *IMAP4.deleteacl()..imaplib.pyx*

   Delete the ACLs (remove any rights) set for who on mailbox.

IMAP4.enable(capability)                 *IMAP4.enable()..imaplib.pyx*
...
IMAP4.expunge()                         *IMAP4.expunge()..imaplib.pyx*
...
IMAP4.fetch(message_set, message_parts)   *IMAP4.fetch()..imaplib.pyx*
...
IMAP4.login(user, password)               *IMAP4.login()..imaplib.pyx*

   Identify the client using a plaintext password. The _password_ will
   be quoted.

                                 *IMAP4.login_cram_md5()..imaplib.pyx*
...
IMAP4.logout()                           *IMAP4.logout()..imaplib.pyx*
...
IMAP4.lsub(directory='""', pattern='*')    *IMAP4.lsub()..imaplib.pyx*
...
IMAP4.myrights(mailbox)                *IMAP4.myrights()..imaplib.pyx*

   Show my ACLs for a mailbox (i.e. the rights that I have on
   mailbox).

IMAP4.namespace()                     *IMAP4.namespace()..imaplib.pyx*

   Returns IMAP namespaces as defined in **RFC 2342**.

IMAP4.noop()                               *IMAP4.noop()..imaplib.pyx*

   Send "NOOP" to server.

IMAP4.open(host, port, timeout=None)       *IMAP4.open()..imaplib.pyx*
...
IMAP4.send(data)                           *IMAP4.send()..imaplib.pyx*
...
IMAP4.starttls(ssl_context=None)       *IMAP4.starttls()..imaplib.pyx*
...
IMAP4.status(mailbox, names)             *IMAP4.status()..imaplib.pyx*

   Request named status conditions for _mailbox_.

                                          *IMAP4.store()..imaplib.pyx*
...
IMAP4.subscribe(mailbox)              *IMAP4.subscribe()..imaplib.pyx*

   Subscribe to new mailbox.

                                         *IMAP4.thread()..imaplib.pyx*
...
IMAP4.uid(command, arg[, ...])              *IMAP4.uid()..imaplib.pyx*
...
IMAP4.unsubscribe(mailbox)          *IMAP4.unsubscribe()..imaplib.pyx*

   Unsubscribe from old mailbox.

IMAP4.unselect()                       *IMAP4.unselect()..imaplib.pyx*
...
IMAP4.xatom(name[, ...])                  *IMAP4.xatom()..imaplib.pyx*
...
IMAP4.PROTOCOL_VERSION           *IMAP4.PROTOCOL_VERSION..imaplib.pyx*

   The most recent supported protocol in the "CAPABILITY" response
   from the server.

IMAP4.debug                                 *IMAP4.debug..imaplib.pyx*

   Integer value to control debugging output.  The initialize value is
   taken from the module variable "Debug".  Values greater than three
   trace each command.

IMAP4.utf8_enabled                   *IMAP4.utf8_enabled..imaplib.pyx*
