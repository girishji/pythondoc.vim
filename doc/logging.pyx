*logging.pyx*                                 Last change: 2023 Sep 15
...
class logging.Logger                             *Logger..logging.pyx*

   propagate                              *Logger.propagate..logging.pyx*
   ...
   setLevel(level)                       *Logger.setLevel()..logging.pyx*
   ...
   isEnabledFor(level)               *Logger.isEnabledFor()..logging.pyx*
   ...
   getEffectiveLevel()          *Logger.getEffectiveLevel()..logging.pyx*
   ...
   getChild(suffix)                      *Logger.getChild()..logging.pyx*
   ...
   getChildren()                      *Logger.getChildren()..logging.pyx*
   ...
   debug(msg, *args, **kwargs)              *Logger.debug()..logging.pyx*
...
<
   ...
   addFilter(filter)                    *Logger.addFilter()..logging.pyx*

      Adds the specified filter _filter_ to this logger.

   removeFilter(filter)              *Logger.removeFilter()..logging.pyx*

      Removes the specified filter _filter_ from this logger.

   filter(record)                          *Logger.filter()..logging.pyx*
   ...
   addHandler(hdlr)                    *Logger.addHandler()..logging.pyx*

      Adds the specified handler _hdlr_ to this logger.

   removeHandler(hdlr)              *Logger.removeHandler()..logging.pyx*

      Removes the specified handler _hdlr_ from this logger.

                                    *Logger.findCaller()..logging.pyx*
   ...
   handle(record)                          *Logger.handle()..logging.pyx*
...
class logging.Handler                           *Handler..logging.pyx*

   __init__(level=NOTSET)               *Handler.__init__()..logging.pyx*
   ...
   createLock()                       *Handler.createLock()..logging.pyx*
   ...
   acquire()                             *Handler.acquire()..logging.pyx*

      Acquires the thread lock created with "createLock()".

   release()                             *Handler.release()..logging.pyx*

      Releases the thread lock acquired with "acquire()".

   setLevel(level)                      *Handler.setLevel()..logging.pyx*
   ...
   setFormatter(fmt)                *Handler.setFormatter()..logging.pyx*

      Sets the "Formatter" for this handler to _fmt_.

   addFilter(filter)                   *Handler.addFilter()..logging.pyx*

      Adds the specified filter _filter_ to this handler.

   removeFilter(filter)             *Handler.removeFilter()..logging.pyx*

      Removes the specified filter _filter_ from this handler.

   filter(record)                         *Handler.filter()..logging.pyx*
   ...
   flush()                                 *Handler.flush()..logging.pyx*

      Ensure all logging output has been flushed. This version does
      nothing and is intended to be implemented by subclasses.

   close()                                 *Handler.close()..logging.pyx*
   ...
   handle(record)                         *Handler.handle()..logging.pyx*
   ...
   handleError(record)               *Handler.handleError()..logging.pyx*
   ...
   format(record)                         *Handler.format()..logging.pyx*

      Do formatting for a record - if a formatter is set, use it.
      Otherwise, use the default formatter for the module.

   emit(record)                             *Handler.emit()..logging.pyx*
...
class logging.Formatter(fmt=None, datefmt=None, style='%', validate=True, *, defaults=None)
   ...
   New in version 3.2: The _style_ parameter.

   New in version 3.8: The _validate_ parameter.

   New in version 3.10: The _defaults_ parameter.

   format(record)                       *Formatter.format()..logging.pyx*
      ...
      necessary.
                                 ...
                                 *Formatter.formatTime()..logging.pyx*
   ...
   formatException(exc_info)   *Formatter.formatException()..logging.pyx*
   ...
   formatStack(stack_info)         *Formatter.formatStack()..logging.pyx*
      ...
      value.
                                     ...
                                     *BufferingFormatter..logging.pyx*
class logging.BufferingFormatter(linefmt=None)
   ...
   outputs the event message) is used as the line formatter.
                      ...
                      *BufferingFormatter.formatHeader()..logging.pyx*
   formatHeader(records)
      ...
      records, a title or a separator line.
                      ...
                      *BufferingFormatter.formatFooter()..logging.pyx*
   ...
   format(records)             *BufferingFormatter.format()..logging.pyx*
...
class logging.Filter(name='')                    *Filter..logging.pyx*
   ...
   filter(record)                          *Filter.filter()..logging.pyx*
...
class logging.LogRecord(name, level, pathname, lineno, msg, args, exc_info, func=None, sinfo=None)
   ...
   getMessage()                     *LogRecord.getMessage()..logging.pyx*
...
on adding contextual information to your logging output.
                                          ...
                                          *LoggerAdapter..logging.pyx*
class logging.LoggerAdapter(logger, extra, merge_extra=False)

   Returns an instance of "LoggerAdapter" initialized with an
   underlying "Logger" instance, a dict-like object (_extra_), and a
   boolean (_merge_extra_) indicating whether or not the _extra_
   argument of individual log calls should be merged with the
   "LoggerAdapter" extra. The default behavior is to ignore the
   _extra_ argument of individual log calls and only use the one of
   the "LoggerAdapter" instance

   process(msg, kwargs)            *LoggerAdapter.process()..logging.pyx*
   ...
   manager                           *LoggerAdapter.manager..logging.pyx*

      Delegates to the underlying "manager`" on _logger_.

   _log                                 *LoggerAdapter._log..logging.pyx*
...
logging.getLogger(name=None)                *getLogger()..logging.pyx*
...
logging.getLoggerClass()               *getLoggerClass()..logging.pyx*
...
<
logging.getLogRecordFactory()     *getLogRecordFactory()..logging.pyx*
...
logging.debug(msg, *args, **kwargs)             *debug()..logging.pyx*
...
logging.info(msg, *args, **kwargs)               *info()..logging.pyx*

   Logs a message with level "INFO" on the root logger. The arguments
   are interpreted as for "debug()".

logging.warning(msg, *args, **kwargs)         *warning()..logging.pyx*
   ...
   Changed in version 3.13: Remove the undocumented "warn()" function
   which was an alias to the "warning()" function.

logging.error(msg, *args, **kwargs)             *error()..logging.pyx*

   Logs a message with level "ERROR" on the root logger. The arguments
   are interpreted as for "debug()".

logging.critical(msg, *args, **kwargs)       *critical()..logging.pyx*

   Logs a message with level "CRITICAL" on the root logger. The
   arguments are interpreted as for "debug()".

logging.exception(msg, *args, **kwargs)     *exception()..logging.pyx*
...
logging.log(level, msg, *args, **kwargs)          *log()..logging.pyx*

   Logs a message with level _level_ on the root logger. The other
   arguments are interpreted as for "debug()".

logging.disable(level=CRITICAL)               *disable()..logging.pyx*
...
logging.addLevelName(level, levelName)   *addLevelName()..logging.pyx*
...
logging.getLevelNamesMapping()   *getLevelNamesMapping()..logging.pyx*
...
logging.getLevelName(level)              *getLevelName()..logging.pyx*
...
logging.getHandlerByName(name)       *getHandlerByName()..logging.pyx*
...
logging.getHandlerNames()             *getHandlerNames()..logging.pyx*

   Returns an immutable set of all known handler names.

   New in version 3.12.

logging.makeLogRecord(attrdict)         *makeLogRecord()..logging.pyx*
...
logging.basicConfig(**kwargs)             *basicConfig()..logging.pyx*
...
logging.shutdown()                           *shutdown()..logging.pyx*
...
logging.setLoggerClass(klass)          *setLoggerClass()..logging.pyx*
   ...
   use the "logging.getLogger()" API to get your loggers.
                                  ...
                                  *setLogRecordFactory()..logging.pyx*
...
logging.lastResort                           *lastResort..logging.pyx*
...
logging.captureWarnings(capture)      *captureWarnings()..logging.pyx*
