*multiprocessing.shared_memory.pyx*           Last change: 2023 Sep 15
...
serialization/deserialization and copying of data.
                     ...
                     *SharedMemory..multiprocessing.shared_memory.pyx*
class multiprocessing.shared_memory.SharedMemory(name=None, create=False, size=0)
   ...
   close()      *SharedMemory.close()..multiprocessing.shared_memory.pyx*
   ...
   unlink()    *SharedMemory.unlink()..multiprocessing.shared_memory.pyx*
   ...
   buf              *SharedMemory.buf..multiprocessing.shared_memory.pyx*

      A memoryview of contents of the shared memory block.

   name            *SharedMemory.name..multiprocessing.shared_memory.pyx*

      Read-only access to the unique name of the shared memory block.

   size            *SharedMemory.size..multiprocessing.shared_memory.pyx*
...
<
              ...
              *SharedMemoryManager..multiprocessing.shared_memory.pyx*
...
*SharedMemoryManager.SharedMemory()..multiprocessing.shared_memory.pyx*
   SharedMemory(size)

      Create and return a new "SharedMemory" object with the specified
      "size" in bytes.

*SharedMemoryManager.ShareableList()..multiprocessing.shared_memory.pyx*
...
"with" statement’s code block finishes execution.
                    ...
                    *ShareableList..multiprocessing.shared_memory.pyx*
class multiprocessing.shared_memory.ShareableList(sequence=None, \*, name=None)
   ...
   values to only the "int" (signed 64-bit), "float", "bool", "str"
   (less than 10M bytes each when encoded as utf-8), "bytes" (less
   than 10M bytes each), and "None" built-in data types.  It also
   notably differs from the built-in "list" type in that these lists
   can not change their overall length (i.e. no append, insert, etc.)
...
<
            ...
            *ShareableList.count()..multiprocessing.shared_memory.pyx*
   count(value)

      Returns the number of occurrences of "value".

            *ShareableList.index()..multiprocessing.shared_memory.pyx*
   index(value)

      Returns first index position of "value".  Raises "ValueError" if
      "value" is not present.

   format       *ShareableList.format..multiprocessing.shared_memory.pyx*

      Read-only attribute containing the "struct" packing format used
      by all currently stored values.

   shm             *ShareableList.shm..multiprocessing.shared_memory.pyx*
