*pathlib.pyx*                                 Last change: 2023 Sep 15
...
==========
                                   ...
                                   *UnsupportedOperation..pathlib.pyx*
...
class pathlib.PurePath(*pathsegments)          *PurePath..pathlib.pyx*
...
PurePath.parts                           *PurePath.parts..pathlib.pyx*
...
PurePath.pathmod                       *PurePath.pathmod..pathlib.pyx*
...
PurePath.drive                           *PurePath.drive..pathlib.pyx*
...
>
      ...
      '\\\\host\\share'
<
PurePath.root                             *PurePath.root..pathlib.pyx*
...
PurePath.anchor                         *PurePath.anchor..pathlib.pyx*
...
>
      ...
      '\\\\host\\share\\'
<
PurePath.parents                       *PurePath.parents..pathlib.pyx*
...
PurePath.parent                         *PurePath.parent..pathlib.pyx*
...
PurePath.name                             *PurePath.name..pathlib.pyx*
...
>
      ...
      ''
<
PurePath.suffix                         *PurePath.suffix..pathlib.pyx*
...
PurePath.suffixes                     *PurePath.suffixes..pathlib.pyx*
...
>
      ...
      []
<
PurePath.stem                             *PurePath.stem..pathlib.pyx*
...
>
      ...
      'library'
<
PurePath.as_posix()                 *PurePath.as_posix()..pathlib.pyx*
...
>
      ...
      'c:/windows'
<
PurePath.as_uri()                     *PurePath.as_uri()..pathlib.pyx*
...
PurePath.is_absolute()           *PurePath.is_absolute()..pathlib.pyx*
...
<
                              ...
                              *PurePath.is_relative_to()..pathlib.pyx*
...
PurePath.is_reserved()           *PurePath.is_reserved()..pathlib.pyx*
...
PurePath.joinpath(*pathsegments)    *PurePath.joinpath()..pathlib.pyx*
...
<
   ...
   Changed in version 3.13: Support for the recursive wildcard “"**"”
   was added. In previous versions, it acted like the non-recursive
   wildcard “"*"”.

                                 *PurePath.relative_to()..pathlib.pyx*
...
PurePath.with_name(name)           *PurePath.with_name()..pathlib.pyx*
...
<
PurePath.with_stem(stem)           *PurePath.with_stem()..pathlib.pyx*
...
PurePath.with_suffix(suffix)     *PurePath.with_suffix()..pathlib.pyx*
...
<
                               ...
                               *PurePath.with_segments()..pathlib.pyx*
...
class pathlib.Path(*pathsegments)                  *Path..pathlib.pyx*
...
class pathlib.PosixPath(*pathsegments)        *PosixPath..pathlib.pyx*
...
class pathlib.WindowsPath(*pathsegments)    *WindowsPath..pathlib.pyx*
...
classmethod Path.cwd()                       *Path.cwd()..pathlib.pyx*
...
<
classmethod Path.home()                     *Path.home()..pathlib.pyx*
...
Path.stat(*, follow_symlinks=True)          *Path.stat()..pathlib.pyx*
...
Path.exists(*, follow_symlinks=True)      *Path.exists()..pathlib.pyx*
...
Path.expanduser()                     *Path.expanduser()..pathlib.pyx*
...
<
   ...
   Tip:
     ...
     Set _follow_symlinks_ to "True" or "False" to improve performance
   ...
   By default, or when the _follow_symlinks_ keyword-only argument is
   set to "None", this method follows symlinks except when expanding
   “"**"” wildcards. Set _follow_symlinks_ to "True" to always follow
   ...
   Changed in version 3.13: Emits "FutureWarning" if the pattern ends
   with “"**"”. In a future Python release, patterns with this ending
   will match both files and directories. Add a trailing slash to
   match only directories.

Path.group()                               *Path.group()..pathlib.pyx*
   ...
   module is not available. In previous versions,
   "NotImplementedError" was raised.

Path.is_dir(*, follow_symlinks=True)      *Path.is_dir()..pathlib.pyx*
...
Path.is_file(*, follow_symlinks=True)    *Path.is_file()..pathlib.pyx*
...
Path.is_junction()                   *Path.is_junction()..pathlib.pyx*
...
Path.is_mount()                         *Path.is_mount()..pathlib.pyx*
...
Path.is_symlink()                     *Path.is_symlink()..pathlib.pyx*
...
Path.is_socket()                       *Path.is_socket()..pathlib.pyx*
...
Path.is_fifo()                           *Path.is_fifo()..pathlib.pyx*
...
Path.is_block_device()           *Path.is_block_device()..pathlib.pyx*
...
Path.is_char_device()             *Path.is_char_device()..pathlib.pyx*
...
Path.iterdir()                           *Path.iterdir()..pathlib.pyx*
...
>
      ...
      '#!/usr/bin/env python3\n'
<
Path.owner()                               *Path.owner()..pathlib.pyx*
...
Path.read_bytes()                     *Path.read_bytes()..pathlib.pyx*
...
Path.readlink()                         *Path.readlink()..pathlib.pyx*
...
Path.rename(target)                       *Path.rename()..pathlib.pyx*
...
Path.replace(target)                     *Path.replace()..pathlib.pyx*
...
Path.absolute()                         *Path.absolute()..pathlib.pyx*
...
<
Path.resolve(strict=False)               *Path.resolve()..pathlib.pyx*
...
Path.rmdir()                               *Path.rmdir()..pathlib.pyx*

   Remove this directory.  The directory must be empty.

Path.samefile(other_path)               *Path.samefile()..pathlib.pyx*
...
Path.hardlink_to(target)             *Path.hardlink_to()..pathlib.pyx*
...
Path.touch(mode=0o666, exist_ok=True)      *Path.touch()..pathlib.pyx*
...
Path.unlink(missing_ok=False)             *Path.unlink()..pathlib.pyx*
...
Path.write_bytes(data)               *Path.write_bytes()..pathlib.pyx*
