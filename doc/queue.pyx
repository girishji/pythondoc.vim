*queue.pyx*                                   Last change: 2023 Sep 15
...
class queue.Queue(maxsize=0)                        *Queue..queue.pyx*
...
class queue.LifoQueue(maxsize=0)                *LifoQueue..queue.pyx*
...
class queue.PriorityQueue(maxsize=0)        *PriorityQueue..queue.pyx*
...
<
class queue.SimpleQueue                       *SimpleQueue..queue.pyx*
...
Queue.qsize()                               *Queue.qsize()..queue.pyx*
...
Queue.empty()                               *Queue.empty()..queue.pyx*
...
Queue.full()                                 *Queue.full()..queue.pyx*
...
Queue.put(item, block=True, timeout=None)     *Queue.put()..queue.pyx*
...
Queue.put_nowait(item)                 *Queue.put_nowait()..queue.pyx*

   Equivalent to "put(item, block=False)".

Queue.get(block=True, timeout=None)           *Queue.get()..queue.pyx*
...
Queue.get_nowait()                     *Queue.get_nowait()..queue.pyx*
...
Queue.task_done()                       *Queue.task_done()..queue.pyx*
...
Queue.join()                                 *Queue.join()..queue.pyx*
...
SimpleQueue.qsize()                   *SimpleQueue.qsize()..queue.pyx*

   Return the approximate size of the queue.  Note, qsize() > 0
   doesn’t guarantee that a subsequent get() will not block.

SimpleQueue.empty()                   *SimpleQueue.empty()..queue.pyx*

   Return "True" if the queue is empty, "False" otherwise.  If empty()
   returns "False" it doesn’t guarantee that a subsequent call to
   get() will not block.

                                        *SimpleQueue.put()..queue.pyx*
...
SimpleQueue.put_nowait(item)     *SimpleQueue.put_nowait()..queue.pyx*

   Equivalent to "put(item, block=False)", provided for compatibility
   with "Queue.put_nowait()".

                                        *SimpleQueue.get()..queue.pyx*
...
SimpleQueue.get_nowait()         *SimpleQueue.get_nowait()..queue.pyx*
