*random.pyx*                                  Last change: 2023 Sep 15
...
random.seed(a=None, version=2)                    *seed()..random.pyx*
   ...
   types: _NoneType_, "int", "float", "str", "bytes", or "bytearray".

random.getstate()                             *getstate()..random.pyx*

   Return an object capturing the current internal state of the
   generator.  This object can be passed to "setstate()" to restore
   the state.

random.setstate(state)                        *setstate()..random.pyx*
...
random.randbytes(n)                          *randbytes()..random.pyx*
...
random.randrange(stop)                       *randrange()..random.pyx*
...
random.randint(a, b)                           *randint()..random.pyx*

   Return a random integer _N_ such that "a <= N <= b".  Alias for
   "randrange(a, b+1)".

random.getrandbits(k)                      *getrandbits()..random.pyx*
...
random.choice(seq)                              *choice()..random.pyx*
...
random.shuffle(x)                              *shuffle()..random.pyx*
   ...
   Deprecated since version 3.9, removed in version 3.11: The optional
   parameter _random_.

random.sample(population, k, *, counts=None)    *sample()..random.pyx*
...
random.binomialvariate(n=1, p=0.5)     *binomialvariate()..random.pyx*
...
random.random()                                 *random()..random.pyx*

   Return the next random floating point number in the range "0.0 <= X
   < 1.0"

random.uniform(a, b)                           *uniform()..random.pyx*
...
random.triangular(low, high, mode)          *triangular()..random.pyx*
...
random.gammavariate(alpha, beta)          *gammavariate()..random.pyx*
...
<
random.gauss(mu=0.0, sigma=1.0)                  *gauss()..random.pyx*
...
random.lognormvariate(mu, sigma)        *lognormvariate()..random.pyx*
...
random.vonmisesvariate(mu, kappa)      *vonmisesvariate()..random.pyx*
...
random.paretovariate(alpha)              *paretovariate()..random.pyx*

   Pareto distribution.  _alpha_ is the shape parameter.

random.weibullvariate(alpha, beta)      *weibullvariate()..random.pyx*
...
class random.Random([seed])                       *Random..random.pyx*

   Class that implements the default pseudo-random number generator
   used by the "random" module.

   Deprecated since version 3.9, removed in version 3.11: Formerly the
   _seed_ could be any hashable object.  Now it is limited to:
   "NoneType", "int", "float", "str", "bytes", or "bytearray".

class random.SystemRandom([seed])           *SystemRandom..random.pyx*
