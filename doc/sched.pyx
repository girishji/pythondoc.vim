*sched.pyx*                                   Last change: 2023 Sep 15
...
scheduler.cancel(event)                *scheduler.cancel()..sched.pyx*

   Remove the event from the queue. If _event_ is not an event
   currently in the queue, this method will raise a "ValueError".

scheduler.empty()                       *scheduler.empty()..sched.pyx*

   Return "True" if the event queue is empty.

scheduler.run(blocking=True)              *scheduler.run()..sched.pyx*
...
scheduler.queue                           *scheduler.queue..sched.pyx*
