*sys.monitoring.pyx*                          Last change: 2023 Sep 15
...
A tool identifier is an integer and associated name. Tool identifiers
...
identifier. Identifiers are integers in the range 0 to 5.


Registering and using tools
---------------------------

                                   *use_tool_id()..sys.monitoring.pyx*
sys.monitoring.use_tool_id(id: int, name: str) -> None

   Must be called before "id" can be used. "id" must be in the range 0
   to 5 inclusive. Raises a "ValueError" if "id" is in use.

                                  *free_tool_id()..sys.monitoring.pyx*
sys.monitoring.free_tool_id(id: int) -> None

   Should be called once a tool no longer requires "id".

                                      *get_tool()..sys.monitoring.pyx*
sys.monitoring.get_tool(id: int) -> str | None

   Returns the name of the tool if "id" is in use, otherwise it
   returns "None". "id" must be in the range 0 to 5 inclusive.
...
C_RAISE
   Exception raised from any callable, except Python functions (event
   occurs after the exit).

C_RETURN
   Return from any callable, except Python functions (event occurs
   after the return).
...
registered. Events can be turned on or off by setting the events
either globally or for a particular code object.
...
monitored.
                                    ...
                                    *get_events()..sys.monitoring.pyx*
sys.monitoring.get_events(tool_id: int) -> int

   Returns the "int" representing all the active events.

                                    *set_events()..sys.monitoring.pyx*
...
Events can also be controlled on a per code object basis.
                              ...
                              *get_local_events()..sys.monitoring.pyx*
sys.monitoring.get_local_events(tool_id: int, code: CodeType) -> int

   Returns all the local events for "code"

                              *set_local_events()..sys.monitoring.pyx*
...
To register a callable for events call
                             ...
                             *register_callback()..sys.monitoring.pyx*
...
* "CALL", "C_RAISE" and "C_RETURN":
     ...
     arg0: object | MISSING) -> DISABLE | Any"
...
* "RAISE", "RERAISE", "EXCEPTION_HANDLED", "PY_UNWIND", "PY_THROW" and
  "STOP_ITERATION":
     ...
     "func(code: CodeType, instruction_offset: int, exception:
     BaseException) -> DISABLE | Any"
...
* "BRANCH" and "JUMP":
     ...
     destination_offset: int) -> DISABLE | Any"
