*termios.pyx*                                 Last change: 2023 Sep 15
...
termios.tcgetattr(fd)                       *tcgetattr()..termios.pyx*
...
termios.tcsetattr(fd, when, attributes)     *tcsetattr()..termios.pyx*
...
termios.tcsendbreak(fd, duration)         *tcsendbreak()..termios.pyx*
...
termios.tcdrain(fd)                           *tcdrain()..termios.pyx*

   Wait until all output written to file descriptor _fd_ has been
   transmitted.

termios.tcflush(fd, queue)                    *tcflush()..termios.pyx*
...
termios.tcflow(fd, action)                     *tcflow()..termios.pyx*
...
termios.tcgetwinsize(fd)                 *tcgetwinsize()..termios.pyx*
...
termios.tcsetwinsize(fd, winsize)        *tcsetwinsize()..termios.pyx*
